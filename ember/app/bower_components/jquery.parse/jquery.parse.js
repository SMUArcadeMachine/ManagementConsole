...
(function($)
{
	...
	$.fn.parse = function(options,files)
	{
		...
		{
			var completeFunc = complete, errorFunc, jsonFunc;
            ...
            if (isFunction(options.json))
                jsonFunc = function(results, file, inputElem, event) { options.json(results, file, inputElem, event);};
			...
			{
                var ext = App.Utils.ext(f.file.name);
                if(options.validTypes && options.validTypes.indexOf(ext) === -1){
                    options.error('Invalid file type ' + (ext.toUpperCase()), f.file, f.inputElem);
                    return;
                }
				...
				reader.onload = function(event)
				{
                    ...
                    if(ext == 'json'){
                        jsonFunc(text, f.file, f.inputElem, event);
                        return;
                    }
