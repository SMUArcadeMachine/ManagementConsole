...
abstract class REST_Controller extends CI_Controller
{
    ...
    /**
     * The API key used to check authentication.
     *
     * @var string
     */
    protected $_api_key = '';
    protected $_api_uid = '';
    protected $_api_email = '';
    ...
    /**
     * Override check for keys
     *
     * @var string
     */
    protected $is_check_login_method = FALSE;
    ...
    public function __construct()
    {
        ...
        //Retrieve uid and api key from request
        $this->_retrieve_api_key();
        ...
        $this->_valid_api_key = $this->_detect_api_key();
        $this->_allow = $this->auth_override ?: $this->_valid_api_key;
    ...
    private function _retrieve_api_key(){
        $api_key_variable = config_item('rest_key_name');
        if(!empty($this->_args[$api_key_variable])){
            $this->_api_key = $this->_args[$api_key_variable];
        }else{
            $this->_api_key = NULL;
        }
        $api_key_exploded = explode('_',$this->api_key());
        $this->_api_uid = $api_key_exploded[0];
    }
    /**
     * Remap
     *
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('rest_enable_keys') && !$this->auth_override)
        {

            $this->api_levels = $this->config->item('api_levels');
            // Check the limit
            if (config_item('rest_enable_limits') AND !$this->_check_limit($controller_method))
            {
                $this->response(array('error' => 'This API key has reached the hourly limit for this method.'), 401);
            }

            //Get API level
            if(!empty($this->api_levels[$this->router->class]) && is_numeric($this->api_levels[$this->router->class])){
                $level = $this->api_levels[$this->router->class];
            }else if(!empty($this->api_levels[$this->router->class][$this->router->method]) && is_numeric($this->api_levels[$this->router->class][$this->router->method])){
                $level = $this->api_levels[$this->router->class][$this->router->method];
            }else if(!empty($this->api_levels[$this->router->class][$this->router->method][$this->request->method]) && is_numeric($this->api_levels[$this->router->class][$this->router->method][$this->request->method])){
                $level = $this->api_levels[$this->router->class][$this->router->method][$this->request->method];
            }else{
                $level = 0;
            }
            ...
            $authorized OR $this->response(array('error' => 'This API key does not have enough permissions.'), 401);
        }
    ...
    protected function _fire_method($method, $args){
        ...
        try{
            ...
            $message = !empty($return_data['error']) ? $return_data['error'] : '';
            if($index != null) $return_data['index'] = $index;
            backend_log(array_merge($return_data,array(
                'data' => $this->_args
            )),$e);
            unset($return_data['log_error']);
            $this->_force_flush($message);
            $this->response($return_data,400);
        }catch(Data_error $e){
            if($transacting) q('ROLLBACK');
            $return_data = $e->getData();
            $message = !empty($return_data['error']) ? $return_data['error'] : '';
            ...
            $return_data =  array('error' =>  $message);
            if($index != null) $return_data['index'] = $index;
            backend_log(array_merge($return_data,array(
                'data' => $this->_args
            )),$e);
    ...
    protected function _detect_api_key(){
        ...
        if ($this->api_key()){
            ...
            $sql = $this->db->from($table)->where(array('uid' => $uid,'active' => 1))->get_compiled_select();
            $rows = q(array(
                'sql' => $sql
            ));
            ...
            $count = count($rows);
            for($i = $count - 1;$i >= 0;$i--){
                $hash = $rows[$i][$this->key_column];
                if(password_verify($this->api_key(),$hash)){
                    $valid_login = true;
                    $row = $rows[$i];
                    break;
                }
            ...
            $this->rest->key = $hash;
            $this->rest->uid = $row['uid'];
            $this->rest->level = $row['level'];
            global $admin_login;
            $admin_login = filter_var($row['admin_login'], FILTER_VALIDATE_BOOLEAN);

            $this->rest->ignore_limits = $row['ignore_limits'];
            $this->_api_email = $row['email'];

            if(time() - strtotime($row['date_created']) > 60*60*24*LOGIN_SESSION_DAYS){
                return FALSE;
            }
            /*
    ...
    protected function _parse_put()
    {
        ...
        {
            $this->_put_args = json_decode(file_get_contents('php://input'));
//            parse_str(file_get_contents('php://input'), $this->_put_args);
    ...
    protected function _parse_delete()
    {
        ...
        {
            $this->_delete_args = json_decode(file_get_contents('php://input'));
...
//        $this->_delete_args = json_decode(file_get_contents('php://input'));
    ...
    public function user_type(){
        return $this->rest->level;
    }
    public function email(){
        return $this->_api_email;
    }
